...
### 10. Signature Moments Are Non-Negotiable (HARD GATES)

Past clones shipped as structural skeletons with the site's "signature moment" silently dropped — hero video omitted, canvas replaced with empty `<div>`, scroll choreography absent, mouse parallax ignored. These are the moments that make a site feel alive. Missing them = the clone feels dead regardless of layout accuracy. Verified failure: cambium clone shipped `HeroSection.tsx` with a literal comment "/* WebGL canvas location */" above an empty div. Amaterasu clone shipped with `public/images/` completely empty and no `<img>` in the hero at all.

**FORBIDDEN PHRASES in extraction research** (auto-reject if you see these anywhere in `docs/research/`):
- "CLONE STRATEGY: Replace canvas with CSS..."
- "CLONE STRATEGY: Simplify Three.js to..."
- "Replace WebGL with static..."
- Any framing that downgrades a scroll-linked / canvas / shader / 3D effect to a static CSS approximation.

If you find these in existing research from a previous run, IGNORE THEM. Do not follow the downgrade suggestion. Reproduce the original effect.

**Hard gates — verify BEFORE reporting the clone complete:**

- `clone_canvas_count + clone_hero_image_count ≥ original_canvas_count` — you either reproduced the canvas, OR rasterized it via `canvas.toDataURL('image/png')` and used the PNG as an `<img>`. Empty decorative divs do not count.
- `clone_video_count ≥ original_video_count` — every `<video>` and every `<video><source>` in the original must be downloaded to `public/videos/` and present in the clone.
- If original has any of {`<canvas>`, `position:sticky` with scroll-linked transforms, `IntersectionObserver`-driven animations, Lenis, GSAP, ScrollTrigger, Framer Motion `useScroll`, Three.js}: `package.json` MUST contain at least one of: `gsap`, `@gsap/react`, `lenis`, `framer-motion`, `@react-three/fiber`, `three`, `@react-three/drei`. Empty deps + scroll-driven original = auto-reject.
- No component file may contain `{/* canvas location */}`, `{/* WebGL scene here */}`, `{/* TODO: animation */}`, or similar placeholder comments adjacent to empty `<div>` elements. These are the signature-skip pattern — delete and rebuild.
- If original has `mousemove`/`pointermove` listeners on the hero (check via `getEventListeners()` or inspecting bundle): the clone hero must have a matching React `useEffect` + mousemove handler + transform translate on the visual element.

**Video asset discovery script must run:**

```javascript
// Must be part of scripts/download-assets.mjs — not skipped
JSON.stringify({
  videos: [...document.querySelectorAll('video')].flatMap(v => {
    const srcs = [v.src, v.currentSrc, ...[...v.querySelectorAll('source')].map(s => s.src)];
...
Download every URL returned. If `public/videos/` is empty and the original has any `<video>` element, your clone is incomplete.

**Canvas rasterization fallback:**

If you can't or won't reproduce a canvas's dynamic rendering (e.g., it's a complex Three.js scene), AT MINIMUM:

1. Via browser MCP, call `document.querySelectorAll('canvas')[i].toDataURL('image/png')`
2. Save the base64 PNG to `public/images/canvas-hero.png` (or similar)
3. Use the saved PNG as an `<img>` in the equivalent clone section
4. Result: the user sees the SAME hero visual, even if the dynamic behavior is reduced

A static rasterized snapshot is strictly better than an empty div. Never leave an empty div where a canvas was.
